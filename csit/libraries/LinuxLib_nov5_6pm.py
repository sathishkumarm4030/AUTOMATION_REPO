...
class LinuxLib:
    ...
    def create_PS_and_DG(self):
        ...
        time.sleep(5)
        self.post_operation(template_url + "/deploy/" + self.data_dict['PS_TEMPLATE_NAME'] + "?verifyDiff=true",
                            headers3,
    ...
    def send_commands_and_expect(self, cmds, expect_string="\$"):
        for cmd in cmds.split("\n"):
            print cmd
            output = self.VM_nc.send_command_expect(cmd, expect_string=expect_string, strip_prompt=False,
                                                 strip_command=False)
        logger.info(output, also_console=True)
        return output
